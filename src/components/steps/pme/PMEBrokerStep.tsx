...
import { BaseStepProps, BrokerData } from '../../../types/base';
import { useBroker } from '../../../hooks/base';
...
export default function PMEBrokerStep({ onBack, onSubmit }: BaseStepProps) {
  const { brokerData, updateBrokerData } = useBroker();
    ...
    if (unformattedDoc.length === 11 || unformattedDoc.length === 14) {
      ...
      try {
        ...
        if (data) {
          ...
          updateBrokerData('email', data.email || '');
          updateBrokerData('whatsapp', data.whatsapp || '');
          updateBrokerData('equipe_nome', data.equipe_nome || '');
          updateBrokerData('id', data.id);
        } else {
  ...
  useEffectCleanup(() => {
    ...
    return () => {
      ...
      const notificationElements = document.querySelectorAll('.notification-element');
              ...
              <FormField label="Nome Completo">
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => updateBrokerData('name', e.target.value)}
              ...
              <FormField label="E-mail">
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => updateBrokerData('email', e.target.value)}
              ...
              <FormField label="WhatsApp">
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => updateBrokerData('whatsapp', formatPhone(e.target.value))}
